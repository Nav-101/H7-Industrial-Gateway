...
The schematic has now been translated into a complete **10-layer mixed-signal PCB layout** for the STM32H7 Industrial Gateway.
...
- STM32H743 BGA microcontroller
- External SDRAM and parallel NOR flash
- USB High-Speed through an external ULPI PHY
- 10/100 Ethernet through an RMII PHY
- CAN and RS-485 industrial interfaces
- Precision sigma-delta ADC and geophone front end
- Audio codec
- Multi-rail switching and linear power supplies

The layout was developed around signal integrity, power integrity, return-current continuity, manufacturability and practical board bring-up.

## Placement Strategy

The STM32H7 is positioned near the centre of the PCB, with the external SDRAM and NOR flash placed close to the MCU to reduce FMC bus length and routing congestion.

Other subsystems were grouped into clear functional regions:
...
- ADC and geophone circuitry separated from memory buses and switching regulators
- Audio circuitry grouped around the codec
- Reset, boot, status and debug controls kept accessible

## Stackup and Routing

A **10-layer stackup** was selected to provide enough routing capacity for the BGA processor and parallel memory buses while maintaining continuous reference planes.

The layout includes:

- Controlled-impedance USB and Ethernet differential pairs
- Dedicated internal routing for SDRAM and NOR flash
- Length and skew control for timing-critical memory signals
- Short clock routes with continuous reference planes
- Ground-return vias near critical layer transitions
- Separate design constraints for power, analogue, memory and high-speed interfaces

## Power Distribution

The power system includes protected input power, switching conversion, low-noise regulation and local subsystem filtering.

Regulator placement was driven by current-loop geometry, with input capacitors, switching components, inductors and output capacitors positioned to minimise high-current switching loops.

The PDN uses:

- Dedicated power and ground planes
- Local decoupling at device power pins
- Bulk capacitance at regulators and major loads
- Bottom-side decoupling beneath the MCU
- Multiple power and ground vias
- Filtered supplies for analogue and communication circuitry

## BGA and Memory Routing

The STM32H7 BGA required structured fanout, controlled neck-down and careful routing-layer allocation.

The SDRAM and NOR flash interfaces were routed as separate timing-related groups rather than applying one arbitrary length-matching rule to the entire FMC bus.

Clock, data, address and control signals were constrained according to their electrical function and timing requirements.

## Mixed-Signal Design

The board combines high-speed digital circuitry, switching regulators and low-level analogue acquisition.

Noise control was achieved through:
...
- Separation from switching nodes and memory buses

## Current Status

The physical layout is complete and is now undergoing final verification before manufacturing release.

Remaining checks include:

- ERC and DRC review
- Footprint and pin-number verification
- Memory length and skew checks
- Differential-pair review
- Power-plane and return-path inspection
- Gerber, drill, BOM and placement-file verification

The next stage is fabrication, assembly and structured board bring-up.
## Project Objective
